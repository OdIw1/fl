function varargout=RK4IP_localerr(alpha,beta,gamma,steep,t_raman,...
    T0,P0,m_shape,C0,...
    T,nt,L,nz,...
...
disp1=exp((1i*0.5*beta2*w.^2-alpha/2+1i*w.^3*beta3/6)*h);     % corrected 29.09.2024
disp2=exp((1i*0.5*beta2*w.^2-alpha/2+1i*w.^3*beta3/6)*h/2);
...
while z<L
    ...
    if err>1
        ...
        disp1=exp((1i*0.5*beta2*w.^2-alpha/2+1i*w.^3*beta3/6)*h);   % corrected 29.09.2024, was h/2 there  
        disp2=exp((1i*0.5*beta2*w.^2-alpha/2+1i*w.^3*beta3/6)*h/2); 
        % disp2=sqrt(disp1);
    else 
